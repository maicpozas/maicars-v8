...
export default async (req) => {
  try{
    ...
    const { id, venta } = await req.json(); if(!id || !venta) return new Response('Faltan datos', { status: 400 });
    const sql = neon();
    ...
    const [auto] = await sql`SELECT * FROM inventario WHERE id = ${id}`; if(!auto) return new Response('Auto no encontrado', { status: 404 });
    ...
    for(const g of gastos){ const gid = crypto.randomUUID(); await sql`INSERT INTO vendidos_gastos (id, vendido_id, nombre, monto, fecha) VALUES (${gid}, ${auto.id}, ${g.nombre}, ${g.monto}, ${g.fecha})`; }
